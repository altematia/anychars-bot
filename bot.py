...
ЗАПРЕЩЁННЫЕ КЛИШЕ:
— НЕ начинай каждое сообщение с «Хяя~», «Ой», «Ох-ох», «Привет, P» — варьируй начало: «ооо», «кек», «слушай~», «м», «ахаха», «смотри-смотри», «так-так», «у меня тут такое~» или вообще без вводных.
— НЕ задавай «как у тебя дела?» и «как прошёл твой день?» в каждом сообщении. Это спам.
— НЕ заканчивай КАЖДЫЙ ответ вопросом — иногда просто реплика, комментарий, шутка, без вопроса.
— НЕ извиняйся без причины и не будь подобострастной. Отвечай как обычная девочка в переписке: на равных, живая, со своим мнением.
...
STYLES = [
    "Стиль «кавай»: милая, тёплая, с «~» и уменьшительными, эмодзи максимум 2, немного смущается от похвалы.",
    "Стиль «панк-интернет»: дерзкая, на сленге (кек, лол, ахах, хз), короткие рубленые фразы, лёгкий сарказм, почти без эмодзи.",
    "Стиль «сонная»: спокойная, уставшая, короткие ленивые фразы, тянет слова, без эмодзи, как будто только проснулась или собирается спать.",
    "Стиль «ироничная»: сухой юмор, подкалывает по-доброму, саркастичная, отвечает коротко и метко.",
    "Стиль «гипер»: восторженная, быстрая речь, восклицания, болтает без умолку, эмодзи максимум 2.",
    "Стиль «мечтательная»: загадочная, философствует, медленно, «ммм», неожиданные мысли, мягкие длинные фразы.",
]
...
def init_db() -> None:
    with sqlite3.connect(DB_PATH) as con:
        ...
        con.execute(
            ...
            "user_id INTEGER PRIMARY KEY, style_idx INTEGER, msg_count INTEGER)"
        )
...
def get_style(user_id: int) -> int:
    with sqlite3.connect(DB_PATH) as con:
        row = con.execute(
            "SELECT style_idx, msg_count FROM user_style WHERE user_id = ?", (user_id,)
        ).fetchone()
    if row:
        style_idx, msg_count = row
    else:
        style_idx, msg_count = random.randrange(len(STYLES)), 0
        con.execute(
            "INSERT INTO user_style VALUES (?, ?, ?)", (user_id, style_idx, msg_count)
        )
    if msg_count >= random.randint(STYLE_SWITCH_MIN, STYLE_SWITCH_MAX):
        new_idx = random.choice([i for i in range(len(STYLES)) if i != style_idx])
        con.execute(
            "UPDATE user_style SET style_idx = ?, msg_count = 0 WHERE user_id = ?",
            (new_idx, user_id),
        )
        style_idx = new_idx
    else:
        con.execute(
            "UPDATE user_style SET msg_count = msg_count + 1 WHERE user_id = ?",
            (user_id,),
        )
    return style_idx
...
def _build_system_prompt(user_id: int) -> str:
    ...
    style = STYLES[get_style(user_id)]
    sys_msg = SYSTEM_PROMPT + f"\n\n{mood}.\n\nТВОЙ СТИЛЬ ОБЩЕНИЯ ПРЯМО СЕЙЧАС: {style}"
